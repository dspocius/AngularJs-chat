        ...
        return function(data, scope){
            var thisView = function(){
                ...
                this.initializeDefaultMenu = function(){
                    ...
                    this.addMenuItemInnerLeft({_id:'closeMenu', div:'rightitem',glyphicon:'glyphicon-circle-arrow-right',name:'closeMenu'});
                    var auth = new authentication();
                    auth.connectedUser(function(data){
                        for(var i=0; i < data.programs.length; i++){
                            this.addMenuItemInnerLeft({_id:data.programs[i]._id, link:'/files/'+data.programs[i]._id+'/index.html',name:data.programs[i].name});
                        }
                        this.addMenuItemInnerLeft({_id:'Logout', link:'#logout',name:'Logout'});
                    }.bind(this));
                }
